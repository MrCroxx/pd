...
func (l *balanceLeaderScheduler) createOperator(plan *balancePlan) []*operator.Operator {
	...
	op, err := operator.CreateTransferLeaderOperator(BalanceLeaderType, plan.cluster, plan.region, plan.region.GetLeader().GetStoreId(), []uint64{plan.TargetStoreID()}, operator.OpLeader)
	if err != nil {
