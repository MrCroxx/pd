...
// TransferLeader is an OpStep that transfers a region's leader to one of the candidates.
...
func (tlv2 TransferLeaderV2) String() string {
	return fmt.Sprintf("transfer leader from store %v to one of the stores in %v", tlv2.FromStore, tlv2.Candidates)
}
...
func (tlv2 TransferLeaderV2) Influence(opInfluence OpInfluence, region *core.RegionInfo) {
	...
	to := opInfluence.GetStoreInfluence(tlv2.ToStore)
